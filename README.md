...
A compact native desktop client for Merriam-Webster's Collegiate Dictionary and Collegiate Thesaurus APIs, plus no-key dictionary definitions supplied by Datamuse.
...
Press `Ctrl+F` to focus the search field. Press `Enter` to search all three services, then move between the Dictionary, Thesaurus, and Datamuse tabs. Datamuse does not require a key.
